...
@Component({
  ...
  templateUrl: './changementmanager.component.html',
  styleUrls: ['./changementmanager.component.scss'],
  encapsulation: ViewEncapsulation.None
})
export class ChangementmanagerComponent implements OnInit {
  // Private
  ...
  Approve(id: any) {
    ...
    const changement = this.ChangementRows.find(changement => changement.id === id);

    if (changement) {
    const formData = new FormData();
    formData.append('statutChangement', 'TERMINE');
    formData.append('description', changement.description);
    formData.append('categorie', changement.categorie);
    formData.append('priorite', changement.priorite);
    formData.append('dateCreation', changement.dateCreation);
    formData.append('dateDebutPrevu', changement.dateDebutPrevu);
    formData.append('dateFinPrevu', changement.dateFinPrevu);
    formData.append('commentaire', changement.commentaire);
    formData.append('motif', changement.motif);
    formData.append('etapesMiseEnOeuvre', changement.etapesMiseEnOeuvre);
    formData.append('actionsCorrectives', changement.actionsCorrectives);
    formData.append('evaluationRisques', changement.evaluationRisques);
    formData.append('impactUtilisateurs', changement.impactUtilisateurs);

    this.changementService.updateChangement(id, formData).subscribe(
        response => {
          console.log('Status updated to APPROUVE successfully', response);
          this.loadChangements();
          this.changementService.notifyAccepted(id).subscribe()
        },
        error => {
