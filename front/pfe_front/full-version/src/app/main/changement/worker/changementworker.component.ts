...
@Component({
  ...
  templateUrl: './changementworker.component.html',
  styleUrls: ['./changementworker.component.scss'],
  encapsulation: ViewEncapsulation.None
})
export class ChangementworkerComponent implements OnInit {
  // Private
  ...
  loadChangements(): void {
    this.changementService.getAllChangementsFromUser(this.userId).subscribe((data: Changement[]) => {
      this.rows = data;
      this.tempData = [...data];
      console.log(data)
  ...
  loadAffectedIssues() : void {
    this.userService.getAffectedIssues(this.userId).subscribe((data:any)=>{
      data.Problemes.forEach(problem=>{
  ...
  editChangement() {
    ...
    formData.append('evenementChangement', 'DEMANDER');
    formData.append('priorite', formValues.priorite);
    formData.append('statutChangement', changement.statutChangement);
