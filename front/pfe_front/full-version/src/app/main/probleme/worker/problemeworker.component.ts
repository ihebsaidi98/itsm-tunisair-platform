...
@Component({
  selector: 'app-problemeworker',
  templateUrl: './problemeworker.component.html',
  styleUrls: ['./problemeworker.component.scss'],
  encapsulation: ViewEncapsulation.None
})
export class ProblemeworkerComponent implements OnInit {
  // Private
  ...
  loadProblems(): void {
    this.rows = []
    this.problemeService.findProblemesByUserId(this.userId).subscribe((data: Probleme[]) => {
      data.forEach((problem)=>{
        if(!problem.archived){
          this.rows.push(problem)
        }
      })
